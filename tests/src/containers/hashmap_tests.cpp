...
test_creation_and_deletion()
{
    ...
    int value = 42;
    expect_should_be(true, map.add(STR("answer"), &value));
    expect_should_be(1, map.count);

    expect_should_be(true, map.remove(STR("answer")));
    expect_should_be(0, map.count);
...
test_add_find_remove()
{
    ...
    expect_should_be(true, map.add(STR("a"), &v1));
    expect_should_be(true, map.add(STR("b"), &v2));
    expect_should_be(2, map.count);

    // Test find_ptr
    int *out_ptr = nullptr;
    expect_should_be(true, map.find_ptr(STR("a"), &out_ptr));
    expect_should_be(v1, *out_ptr);

    // Test find
    int out_copy = 0;
    expect_should_be(true, map.find(STR("a"), &out_copy));
    expect_should_be(v1, out_copy);

    expect_should_be(true, map.remove(STR("a")));
    expect_should_be(1, map.count);

    CORE_DEBUG(
        "The next 2 warnings about a missing key are expected (lookup after "
        "removal).");
    expect_should_be(false, map.find_ptr(STR("a"), &out_ptr));
    expect_should_be(false, map.find(STR("a"), &out_copy));

    // Test both find methods on remaining element
    expect_should_be(true, map.find_ptr(STR("b"), &out_ptr));
    expect_should_be(v2, *out_ptr);

    out_copy = 0;
    expect_should_be(true, map.find(STR("b"), &out_copy));
...
test_full_hashmap_does_not_change_size()
{
    ...
    expect_should_be(true, map.add(STR("one"), &values[0]));
    expect_should_be(true, map.add(STR("two"), &values[1]));
    expect_should_be(true, map.add(STR("three"), &values[2]));
    expect_should_be(true, map.add(STR("four"), &values[3]));

    expect_should_be(map.capacity, map.count);

    CORE_DEBUG("The next warning about a full hashmap is expected.");
    expect_should_be(false, map.add(STR("overflow"), &values[4]));
    expect_should_be(map.capacity, map.count);
...
test_debug_log_showcase()
{
    ...
    expect_should_be(true, map.add(STR("one"), &v1));
    expect_should_be(true, map.add(STR("two"), &v2));
    expect_should_be(true, map.add(STR("three"), &v3));

    CORE_DEBUG("HashMap debug log (after insertions) follows:");
    map.debug_log_table();

    expect_should_be(true, map.remove(STR("two")));
    expect_should_be(2, map.count);
...
test_init_and_shutdown()
{
    ...
    // Add some data
    int value = 100;
    expect_should_be(true, map.add(STR("test"), &value));
    expect_should_be(1, map.count);
...
test_find_ptr_vs_find()
{
    ...
    expect_should_be(true, map.add(STR("test"), &original_value));

    // Test find_ptr returns a pointer to the internal value
    int *ptr_result = nullptr;
    expect_should_be(true, map.find_ptr(STR("test"), &ptr_result));
    expect_should_be(100, *ptr_result);

    // Test find returns a copy of the value
    int copy_result = 0;
    expect_should_be(true, map.find(STR("test"), &copy_result));
    ...
    *ptr_result = 200;
    ...
    expect_should_be(true, map.find_ptr(STR("test"), &ptr_check));
    ...
    int new_copy = 0;
    expect_should_be(true, map.find(STR("test"), &new_copy));
    expect_should_be(200, new_copy);

    // Modify the copy should NOT affect the hashmap
    new_copy       = 300;
    int *final_ptr = nullptr;
    expect_should_be(true, map.find_ptr(STR("test"), &final_ptr));
...
test_operations_before_init()
{
    ...
    expect_should_be(false, map.add(STR("key"), &value));

    // Try to find_ptr before init
    expect_should_be(false, map.find_ptr(STR("key"), &out_ptr));

    // Try to find before init
    expect_should_be(false, map.find(STR("key"), &out_copy));

    // Try to remove before init
    expect_should_be(false, map.remove(STR("key")));
    ...
    map.init(test_arena, 4);
    expect_should_be(true, map.add(STR("key"), &value));
    expect_should_be(true, map.find_ptr(STR("key"), &out_ptr));
    expect_should_be(value, *out_ptr);

    out_copy = 0;
    expect_should_be(true, map.find(STR("key"), &out_copy));
...
test_add_with_overwrite()
{
    ...
    // Add initial value
    expect_should_be(true, map.add(STR("key"), &original_value));
    expect_should_be(1, map.count);

    // Verify initial value
    int *ptr_result = nullptr;
    expect_should_be(true, map.find_ptr(STR("key"), &ptr_result));
    expect_should_be(100, *ptr_result);

    CORE_DEBUG("The next warning about duplicate key is expected.");
    // Try to add without overwrite flag (should fail)
    expect_should_be(false, map.add(STR("key"), &new_value, false));
    expect_should_be(1, map.count);
    ...
    expect_should_be(true, map.add(STR("key"), &new_value, true));
    ...
    expect_should_be(true, map.add(STR("another_key"), &another_value, true));
    expect_should_be(2, map.count);

    expect_should_be(true, map.find_ptr(STR("another_key"), &ptr_result));
