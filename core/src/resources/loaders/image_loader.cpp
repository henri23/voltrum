...
Image_Load_Result image_loader_load(const char* image_name) {
    ...
    // Commented out for UI rewrite - embedded image loading disabled
    // const Embedded_Image_Asset* asset = find_image_asset(image_name);
    // if (!asset) {
    result.success = false;
    result.error_message = "Embedded image loading temporarily disabled for UI rewrite";
    CORE_WARN("Image loader called for '%s' but embedded images are disabled during UI rewrite", image_name);
    return result;
    // }
