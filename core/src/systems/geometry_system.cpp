...
void geometry_system_shutdown() {
    // NOTE: Nothing to free beside the geometry reference buffer itself,
    // because the Geometry object holds only the information about the material
    // and not the information of the mesh itself

    // The material system autoreleases all the registered materials on shutdown
    u32 max_count = state.config.max_geometry_count;

    memory_deallocate(state.registered_geometries,
        sizeof(Geometry_Reference) * max_count,
        Memory_Tag::GEOMETRY);
}
...
Geometry* geometry_system_acquire_by_id(Geometry_ID id) {
    ...
    if (id != INVALID_ID &&
        state.registered_geometries[id].geometry.id != INVALID_ID) {
...
Geometry* geometry_system_acquire_by_config(Geometry_Config config,
    ...
    for (u32 i = 0; i < state.config.max_geometry_count; ++i) {
        if (state.registered_geometries[i].geometry.id == INVALID_ID) {
            ...
            geometry = &state.registered_geometries[i].geometry;
            geometry->id = i;
            break;
        }
    }
    ...
    if (!create_geometry(&state, config, geometry)) {
...
void geometry_release(Geometry* geometry) {
    if (geometry->id != INVALID_ID) {
        ...
        if (ref->reference_count < 1 && ref->auto_release) {
            destroy_geometry(&state, &ref->geometry);
