...
//
// // Internal functions
// INTERNAL_FUNC b8 load_ui_image(const char* image_name, UI_Image_Resource*
// image_resource); INTERNAL_FUNC void draw_titlebar_background(); INTERNAL_FUNC
// void draw_titlebar_gradient(); INTERNAL_FUNC void draw_titlebar_logo();
...
//     ImVec2 size);
//
// INTERNAL_FUNC b8 draw_titlebar_image_button(UI_Image_Resource*
// image_resource,
...
//     UI_Image_Resource* app_icon = state.app_icon.is_valid ? &state.app_icon :
//     nullptr;
//
...
//         (VkDescriptorSet)app_icon->descriptor_set; if (descriptor_set !=
//         VK_NULL_HANDLE) {
...
//         ImVec2 text_pos = ImVec2(logo_pos.x + (logo_size - text_size.x) *
//         0.5f,
...
//     ImGui::EndGroup();
//
//     // Simple approach like Walnut: check if menu group is hovered or any
//     popup
//     // is open
...
//     f32 text_x = state.titlebar_min.x + (titlebar_width - text_size.x) *
//     0.5f;
//
...
//     b8 overlaps = (title_start_x < menu_end_x) && (title_end_x >
//     menu_start_x);
//
//     if (overlaps) {
...
//             is_maximized
//                 ? (state.restore_icon.is_valid ? &state.restore_icon :
//                 nullptr) : (state.maximize_icon.is_valid ?
...
//         // Get descriptor set directly from resource
//         VkDescriptorSet descriptor_set =
//         (VkDescriptorSet)image_resource->descriptor_set; if (descriptor_set
//         != VK_NULL_HANDLE) {
...
//             f32 available_width = size.x * 0.7f;  // 70% of button width for
//             icon f32 available_height = size.y * 0.7f; // 70% of button
//             height for icon
//
//             // Use icons at their natural size (no scaling unless they're too
//             large) f32 scale = 1.0f; // Start with natural size
//
...
//             }
//
//             // Calculate final dimensions (natural size or scaled down if
//             needed) f32 scaled_width = (f32)original_width * scale; f32
//             scaled_height = (f32)original_height * scale;
//
...
//             {
//                 // This is likely a minimize icon (very thin and wide) -
//                 align to bottom vertical_pos = pos.y + size.y - image_size.y
//                 - (size.y * 0.15f); // 15% padding from bottom
//             } else {
//                 // Normal icons - center vertically
//                 vertical_pos = pos.y + (size.y - image_size.y) * 0.5f;
//             }
//
//             ImVec2 image_pos = ImVec2(pos.x + (size.x - image_size.x) * 0.5f,
//             // Center horizontally
//                 vertical_pos);
//
//             // Render the icon with proper color tinting based on button
//             state u32 icon_color = palette.text; if (ImGui::IsItemActive()) {
...
//             draw_list->AddImage((ImTextureID)(intptr_t)descriptor_set,
//                 image_pos,
//                 ImVec2(image_pos.x + image_size.x, image_pos.y +
//                 image_size.y), ImVec2(0, 0), ImVec2(1, 1), icon_color);
//         } else {
//             // GPU resource not ready, use fallback
...
//                 (void*)image_resource,
//                 (image_resource && image_resource->is_valid) ? "true" :
//                 "false");
...
//             bar_rect.Min.x + window->WindowBorderSize + window->Pos.x
//             - 10.0f)),
...
//         ImVec2(bar_rect.Min.x + window->Pos.x, bar_rect.Min.y +
//         window->Pos.y);
//
...
//     if (!result.success) {
//         CORE_ERROR("Failed to load image '%s': %s", image_name,
//         result.error_message); return false;
//     }
//
...
//
//     if (!success) {
//         CORE_ERROR("Failed to create GPU image resource for '%s'",
